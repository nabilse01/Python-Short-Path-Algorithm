...
'''
The code is responsible for measuring the performance and comparing the run-time of A* algorithm and Dijkstra's algorithm for finding the shortest path between randomly selected start and end nodes. The code performs the following steps:

It selects two unique nodes randomly from a node list.
Then, it measures the runtime of both algorithms for computing the shortest path and retrieves the cost and path of the shortest path.
It calculates the elapsed time for each algorithm.
It records the start_node, end_node, shortest_distance, shortest_path, elapsed_time, number_of_nodes_shortest_path in lists named results_astar and results_dijkstra, respectively.
Finally, it writes the results of both algorithms to separate csv files named shortest_paths_astar.csv and shortest_paths_dijkstra.csv.
In summary, this code is a benchmarking tool that can be used to compare the performance of different shortest path algorithms based on their results.
'''
...
# Iterate through id_list and randomly select start and end nodes
# Loop through the range of items in id_list using index i
# Convert all id_list items to strings
id_list = [str(id) for id in id_list]

# Shuffle the id_list
random.shuffle(id_list)

# Keep track of results for A* and Dijkstra algorithms
results_astar = []
results_dijkstra = []


#     # Keep looping until two unique items are randomly selected
# for j in id_list:
#     start = str(j)
#     for h in id_list:
#         end = str(h) 
#     # Measure the runtime of A* algorithm and retrieve the shortest path and cost
#         start_time = time.time()
#         shortest_path_cost, shortest_path = astar(graph, start, end)
#         end_time = time.time()
#         elapsed_time = end_time - start_time  # Calculate elapsed time for A*
#         # calculate length of shortest path
#         number_of_nodes_shortest_path = len(shortest_path)

#         results_astar.append((start, end, shortest_path_cost,
#                             shortest_path, elapsed_time, number_of_nodes_shortest_path))  # Append results of A* in a list

#         # Dijkstra's algorithm
#         start_time = time.time()
#         # Apply dijkstra's algorithm on given graph
#         shortest_path_cost, shortest_path = dijkstra(graph, start, end)
#         end_time = time.time()  # get end time of execution of algorithm
#         # Calculate elapsed time for dijkstra's algorithm
#         elapsed_time = end_time - start_time
#         # Calculate length of shortest distance path
#         number_of_nodes_shortest_path = len(shortest_path)
        
#         results_dijkstra.append((start, end, shortest_path_cost,
#                                 shortest_path, elapsed_time, number_of_nodes_shortest_path))  # Append results of dijkstra's algorithm in a list.

# # Write the results to CSV files
# with open('shortest_paths_astar2.csv', 'w', newline='') as csvfile:
#     writer = csv.writer(csvfile)
#     # Write header row to csv file
#     writer.writerow(['start_node', 'end_node',
#                     'shortest_distance', 'shortest_path', 'elapsed_time', 'number_of_nodes_shortest_path'])
#     # Write each row of results to csv file
#     for start, end, shortest_path_cost, shortest_path, elapsed_time, number_of_nodes_shortest_path in results_astar:
#         writer.writerow([start, end, shortest_path_cost,
#                         shortest_path, elapsed_time, number_of_nodes_shortest_path])

# with open('shortest_paths_dijkstra2.csv', 'w', newline='') as csvfile:
#     writer = csv.writer(csvfile)
#     # Write header row to csv file
#     writer.writerow(['start_node', 'end_node',
#                     'shortest_distance', 'shortest_path', 'elapsed_time', 'number_of_nodes_shortest_path'])
#     # Write each row of results to csv file
#     for start, end, shortest_path_cost, shortest_path, elapsed_time, number_of_nodes_shortest_path in results_dijkstra:
#         writer.writerow([start, end, shortest_path_cost,
#                         shortest_path, elapsed_time, number_of_nodes_shortest_path])
